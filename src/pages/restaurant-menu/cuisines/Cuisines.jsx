...
export default function Cuisines() {
  const { data, reFetch } = GetAPI("admin/getcuisines");
  const restaurant = GetAPI("admin/getallrestaurants");
  ...
  const resOptions = [];
  restaurant?.data?.data?.map((res, index) =>
    resOptions.push({
      value: res?.id,
    ...
    return datas.push({
      ...
      action: (
        <EditButton
          text="Update"
          onClick={() =>
            openModal("Update Cuisine", values?.id, values?.name, values?.path)
          }
        />
      ),
    });
